...
transcriptions: Dict[UUID, TranscriptionRead] = {}
...
def get_transcription(trans_id: UUID):
    """Get single transcription result"""
    if trans_id not in transcriptions:
        raise HTTPException(status_code=404, detail="Transcription not found")
    return transcriptions[trans_id]

@app.put("/transcriptions/{trans_id}")
def update_transcription(trans_id: UUID):
    """Placeholder for updating a transcription"""
...
async def create_transcription(file: UploadFile = File(...)):
    ...
    trans = TranscriptionRead(
        id=trans_id,
        audio_filename=file_name,
        text=text_result,
        status="completed",
    )
    transcriptions[trans_id] = trans
    return trans
